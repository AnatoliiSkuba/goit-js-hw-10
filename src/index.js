...
searchBox.addEventListener("input", debounce((evt) => {
    ...
    let persistedSearch = localStorage.getItem('onSearch');
    persistedSearch = persistedSearch ? JSON.parse(persistedSearch) : "";
    persistedSearch = evt.target.value.trim();
    localStorage.setItem("onSearch", JSON.stringify(persistedSearch));
